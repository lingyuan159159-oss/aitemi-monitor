...
export function SettingsDialog({ open, onOpenChange, refreshInterval, onIntervalChange, scanIntervals, scanTimeRange, onTimeRangeChange, thresholds, onThresholdsChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md rounded-2xl bg-white/90 dark:bg-[#1c1c1e] backdrop-blur-xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-[17px] font-semibold text-[#1d1d1f] dark:text-white">设置</DialogTitle>
        </DialogHeader>

        <div className="space-y-5 overflow-y-auto flex-1 min-0 pr-1">
          <div>
            <Label className="text-xs text-[#86868b] dark:text-[#98989d] font-medium uppercase tracking-wider">页面刷新</Label>
            <Select value={String(refreshInterval)} onValueChange={(v: string | null) => v && onIntervalChange(parseInt(v, 10))}>
              <SelectTrigger className="mt-2 rounded-xl border-black/[0.06] dark:border-white/[0.06] bg-[#f5f5f7] dark:bg-[#2c2c2e] h-10">
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            <Label className="text-xs text-[#86868b] dark:text-[#98989d] font-medium uppercase tracking-wider">采集时间范围</Label>
            <div className="flex items-center gap-2 mt-2">
              <div className="flex-1 space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">开始时间</Label>
                <Input
                  type="time"
                  value={timeRange.start}
                  className="h-9 text-sm rounded-xl border-black/[0.06] dark:border-white/[0.06] bg-[#f5f5f7] dark:bg-[#2c2c2e]"
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTimeRange(prev => ({ ...prev, start: e.target.value }))}
                />
              </div>
              <span className="text-[#86868b] dark:text-[#98989d] mt-5">至</span>
              <div className="flex-1 space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">结束时间</Label>
                <Input
                  type="time"
                  value={timeRange.end}
                  className="h-9 text-sm rounded-xl border-black/[0.06] dark:border-white/[0.06] bg-[#f5f5f7] dark:bg-[#2c2c2e]"
            ...
            <p className="text-[11px] text-[#86868b] dark:text-[#98989d] mt-1.5">超出时间范围将自动暂停采集</p>
          </div>

          <Separator className="bg-black/[0.06] dark:bg-white/[0.06]" />

          <div>
            <Label className="text-xs text-[#86868b] dark:text-[#98989d] font-medium uppercase tracking-wider">采集间隔（分钟）</Label>
            <div className="grid grid-cols-2 gap-3 mt-3">
              {Object.entries(intervalLabels).map(([key, label]) => (
                <div key={key} className="space-y-1">
                  <Label className="text-xs text-[#1d1d1f] dark:text-white">{label}</Label>
                  <Input
                    type="number"
          ...
          <div>
            <Label className="text-xs text-[#86868b] dark:text-[#98989d] font-medium uppercase tracking-wider">超时阈值（分钟）</Label>
            <div className="grid grid-cols-2 gap-3 mt-3">
              <div className="space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">分拣超时</Label>
                <Input
                  type="number"
              ...
              <div className="space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">配送超时</Label>
                <Input
                  type="number"
              ...
              <div className="space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">压单</Label>
                <Input
                  type="number"
              ...
              <div className="space-y-1">
                <Label className="text-xs text-[#1d1d1f] dark:text-white">跳扫码（秒）</Label>
                <Input
                  type="number"
            ...
            <p className="text-[11px] text-[#86868b] dark:text-[#98989d] mt-1.5">超过阈值即判定为异常</p>
          </div>
        </div>
        ...
        <DialogFooter className="gap-2">
          <Button
            ...
            className="rounded-xl border-black/[0.06] dark:border-white/[0.06] bg-[#f5f5f7] dark:bg-[#2c2c2e] text-[#1d1d1f] dark:text-white hover:bg-black/[0.04] dark:hover:bg-white/[0.05]"
          >
            取消
          </Button>
          <Button
